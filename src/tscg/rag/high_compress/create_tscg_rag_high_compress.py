...
NOTES:
  ...
  • Segmentation strategies:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip float32→float16 quantization of embedding BLOBs in chroma.sqlite3. "
            "Quantization saves ~47%% of the sqlite3 embedding column with negligible "
            "precision loss (HNSW index stays float32 and is unaffected)."
        )
    )
...
def quantize_sqlite_embeddings(db_path: str, dim: int = 384, verbose: bool = False) -> Dict:
    """
    Post-process ChromaDB's chroma.sqlite3 to repack float32 embedding BLOBs
    as float16 — cutting the embedding column size by ~50% with negligible
    precision loss (cosine error < 0.0001; HNSW index stays float32).

    ChromaDB architecture note:
      - chroma.sqlite3  → stores raw embedding vectors (BLOBs), documents, metadata
      - data_level0.bin → HNSW index used for all similarity queries (always float32)
    Repacking only affects sqlite3; query performance and ranking are unchanged.

    Args:
        db_path : path to the ChromaDB directory (not the .sqlite3 file itself)
        dim     : embedding dimension (384 for all-MiniLM-L6-v2, 768 for larger models)
        verbose : print progress

    Returns:
        dict with size_before_mb, size_after_mb, rows_updated, saved_mb
    """
    ...
    if not sqlite_path.exists():
        # Try legacy path
        sqlite_path = Path(db_path) / "chroma" / "chroma.sqlite3"
    if not sqlite_path.exists():
    ...
    try:
        # Discover embedding BLOB columns dynamically — works across ChromaDB versions
        ...
        for table in tables:
            ...
            for col in blob_cols:
                # Sample to verify it contains float32 vectors of expected dim
                sample = conn.execute(
                ...
                if not isinstance(blob, bytes):
                    continue
                if len(blob) != dim * 4:   # expect exactly dim × 4 bytes (float32)
                    if verbose:
                        print(f"  Skipping {table}.{col}: "
                              f"len={len(blob)} ≠ {dim*4} (dim={dim})")
                    continue

                # Repack float32 → float16 in a single transaction
                if verbose:
                    count = conn.execute(
                        f"SELECT COUNT(*) FROM {table} WHERE {col} IS NOT NULL"
                    ).fetchone()[0]
                    print(f"  Repacking {table}.{col}: {count} rows × {dim} dims "
                          f"float32→float16...")

                conn.execute("BEGIN")
                updated = 0
                for rowid, blob32 in conn.execute(
                    f"SELECT rowid, {col} FROM {table} WHERE {col} IS NOT NULL"
                ):
                    floats = struct.unpack(f'<{dim}f', blob32)
                    blob16 = struct.pack(f'<{dim}e', *floats)
                    conn.execute(
    ...
    return {
        ...
        "reduction_pct":  round(100 * saved_mb / (size_before / (1024 * 1024)), 1),
    }
...
def main():
    ...
    # 9. SQLite float32→float16 quantization (default: on)
    #    Repacks embedding BLOBs in chroma.sqlite3 from float32 to float16.
    #    Saves ~47% of the embedding column (≈ 20-25% of total DB size).
    #    The HNSW index (data_level0.bin) is NOT modified — queries are unaffected.
    quant_info = ""
    if not args.no_quantize:
        print("\n🔬 Quantizing sqlite3 embeddings float32→float16...")
        # Embedding dim: 384 for all-MiniLM-L6-v2, 768 for larger models
        embedding_dim = 384 if args.mode == "local" else 768
        q = quantize_sqlite_embeddings(args.db, dim=embedding_dim, verbose=args.verbose)
        if "error" in q:
            print(f"  ⚠️  Quantization skipped: {q['error']}")
        else:
            quant_info = (f"  sqlite3: {q['size_before_mb']:.1f} MB → "
                          f"{q['size_after_mb']:.1f} MB  "
                          f"(−{q['saved_mb']:.1f} MB, {q['reduction_pct']}%)")
            print(f"✓ Quantized {q['rows_updated']} embedding rows")
            print(quant_info)

    # 10. Compress database (default: on)
    archive_info = ""
    ...
    if quant_info:
        print(f"  sqlite3 quantization  :{quant_info.strip()}")
    if archive_info:
        print(archive_info)
    if args.dedup_report:
