# TSCG Smart Prompt v16.1.0 - Score Convention Option B & Corpus Expansion
...
**Major Updates v16.0.0→v16.1.0**:
- ✅ **Score Convention Option B**: `A_score/S_score/It_score/D_score/R_score/E_score/V_score/O_score/Im_score` — distinct from MonoidalTypes — 36 M0 jsonld + 31 README migrated
- ✅ **TRIZ normalized**: Altshuller 0–5 → 0–1 (÷5); ASFID mean=0.88, REVOI mean=0.96, δ₁=0.057 (OnCriticalLine)
- ✅ **Corpus**: 24→26 poclets (+ Ptoe, PlateTectonics, NuclearReactorsTypology) + Theremin stub
- ✅ **New SystemicFramework**: Triz — total: 2 (Vsm + Triz)
- ✅ **TscgTools modeled**: TscgOntologyExplorer + TscgPocletGenerator + TscgPocletMiner (m3:TscgTool)
- ✅ **TscgOntologyAPIServer stub**: Phase 3 planned
- ✅ **Python-bridge Phase 2**: TscgStore (pyoxigraph), /corpus/* endpoints, 58 tests
- ✅ **New**: docs/CoreHypotheses/ (8 files), ontology/TSCG_InstanceGrammar/, ontology/InstanceSimulations/, M3_BicephalousPerspective
...
### 7. Score Convention Option B — MANDATORY (v16.1.0)

Score DatatypeProperties are **distinct** from MonoidalTypes:
- `m0:scoreA` ≠ `m3:eagle_eye:Attractor` (MonoidalType)
- `m0:scoreR` ≠ `m3:sphinx_eye:Representable` (MonoidalType)

**@context required in every M0 file:**
```json
...
|---|---|---|---|
| `instances/poclets/` | Single-domain concrete poclets (M0) | **26** | RAAS, FireTriangle, Ptoe, PlateTectonics, NuclearReactorsTypology |
| `instances/symbolic-system-grammars/` | Formal symbolic grammars | **1** | Iching (I Ching hexagram system) |
| `instances/systemic-frameworks/` | External methodologies | **2** | Vsm, Triz (TRIZ icosahedron) |
| `instances/tscg-tools/` | Reflexive tools (TSCG as domain) | **3** (+1 stub) | TscgOntologyExplorer, TscgPocletGenerator, TscgPocletMiner (+ TscgOntologyAPIServer stub) |

**Total M0 Instances**: 33 validated (26 Poclets + 2 SystemicFrameworks + 1 SymbolicSystemGrammar + 3 TscgTools) + 1 stub
...
|------|-----------------|-------------|-------|----------|
| **Poclet** | `m3:Poclet` | Minimal complete validated system model | **26** | RAAS, Ptoe, PlateTectonics, NuclearReactorsTypology, FireTriangle, NakamotoConsensus |
| **SymbolicSystemGrammar** | `m3:SymbolicSystemGrammar` | Formal symbolic grammar systems | **1** | Iching (64 hexagrams) |
| **SystemicFramework** | `m3:SystemicFramework` | Established external methodologies | **2** | Vsm, Triz (scores normalized 0–1) |
