# TSCG Smart Prompt v16.3.0 — Structural Grammar (× + |) + WS-n reorg + SC-3 facet decision
...
**Major Updates v16.2.0→v16.3.0**:
- ✅ **`⊗` RETIRED (forbidden)**: Structural Grammar operators are `×` (Territory/ASFID/Gt), `+` (Map/REVOI/Gm), `|` (Stereopsis/Gs). Any `⊗` in a formula is a defect. (0 occurrences in M2 at HEAD.)
- ✅ **SC-2 complete**: `M2_GenericConcepts.jsonld` v16.19.0 — monoidal formulas, St/It subscripting (S/I always monoid-subscripted; A/F/D bare).
- ✅ **Worksites reorganized to WS-n** under `ontology/docs/_01_Worksite/` (WS-0 Structural Cleanup, WS-3 Gs review, WS-8 Until-Further-Notice). Durable index: `WS-0/_00_TSCG_Worksite_Map.md` v2.2.0.
- 🟡 **SC-3 facet principle DECIDED, not graved** (`SC-3_Facet_Decision_Record.md` v1.0.0): four orthogonal axes SCALE / NATURE / AUDIENCE / DOMAIN, to be lifted from the existing `m0:Facet`. **Until graved, `m3:ontologyType` remains the SINGLE instance-type property** — do not model the 4-axis scheme as live yet.
- ✅ **New instances**: CanopyGraphVizProto (poclet), Bmc (SystemicFramework), TriskeleToolchain (SSG), TscgLayerCakeMCPServer + TscgLittleBigBrain (tools).

**Major Updates v16.1.0→v16.2.0**:
- ✅ **Structural Grammar formalism**: M3 foundation migrated from Hilbert space / tensor product to **Lambek calculus / free commutative monoidal categories**. operators are `×` (Territory/ASFID/Gt), `+` (Map/REVOI/Gm), `|` (Stereopsis/Gs). **`⊗` is RETIRED — forbidden.** Braille diagram (©2022 Michel Kern) = canonical empirical proof.
- ✅ **M3_BicephalousPerspective.jsonld v1.5.1**: Gs monoid (|, TKSL), St/It indexation, Pole Annihilation Axiom, TriadicBalance `_0 = _^ | _$`
- ✅ **M2_GenericConcepts.jsonld v16.19.0**: 80 atomic patterns (+ `m2:Modelisation`, `m2:TriadicBalance`)
...
- ✅ **M1_CoreConcepts.jsonld v2.9.1**: `m1:Permeability` validated as M1 GenericConceptCombo — v2.9.1 at HEAD
...
- ✅ Corpus: 27 poclet dirs (+ CanopyGraphVizProto, typed m3:Poclet)
...
M3_GenesisGrammar.jsonld      ← Mathematical foundation (Lambek calculus / monoidal categories)
  ...
  └── M3_BicephalousPerspective (Gs, |, TKSL — Stereopsis)
        ↓ imports
M2_GenericConcepts.jsonld     ← atomic patterns (10 families incl. Combo)
...
⚠️ **`⊗` is RETIRED (forbidden).** The monoidal product is written `×` (Gt), `+` (Gm), `|` (Gs). No metric required. Lambek calculus: free commutative monoidal categories.
...
**Status**: atomic GenericConcepts across 10 families (strictly transdisciplinary, ≥6 domains each). Family spread at HEAD: Structural 23, Dynamic 17, Ontological/Regulatory 10, Informational 8, Combo 6, Relational 5, Adaptive/Energetic 4, Teleonomic 1.
...
|------|---------|---------|
| `M1_Biology.jsonld` | v1.2.0 | 24 concepts + KFCombo instances |
| `M1_Chemistry.jsonld` | v1.2.0 | Bond types, reaction kinetics |
| `M1_Physics.jsonld` | v1.2.0 | Thermodynamics, quantum |
| `M1_Electronics.jsonld` | v1.1.0 | Analog circuits, semiconductors |
| `M1_Economics.jsonld` | v1.1.0 | Financial crisis phases |
| `M1_Optics.jsonld` | v1.2.0 | Light interaction modes |
| `M1_Photography.jsonld` | v1.2.0 | Exposure triangle concepts |
| `M1_Music.jsonld` | v1.1.0 | Counterpoint, harmony |
| `M1_Mythology.jsonld` | v1.2.0 | Cosmological structures |
| `M1_SystemicModeling.jsonld` | v1.2.0 | VSM, cybernetics |
| `M1_Education.jsonld` | v1.1.0 | TPACK framework |
| `M1_Geology.jsonld` | v1.1.0 | Plate tectonics |
...
3. **M3 formalism**: Lambek calculus / monoidal product `×` `+` `|` — NOT Hilbert space / tensor product; `⊗` is RETIRED
...
12. **UTF-8**: Mathematical symbols (×, +, |, →, Φ, Ψ, _^, _$) — verify after edits (`⊗` must NOT appear)
...
- `M3_BicephalousPerspective.jsonld` — Gs stereopsis grammar (TKSL, poles, TriadicBalance) v1.5.1
- `M2_GenericConcepts.jsonld` — atomic patterns + Combo classes (10 families) v16.19.0
- `M1_CoreConcepts.jsonld` — GenericConceptCombo instances + KF roots v2.9.1
- `M1_Domains.jsonld` — 19 registered domains
...
*Smart Prompt v16.3.0 — 2026-07-30. Anchored on HEAD d92ded2. Prior v16.2.0 taught `⊗` as a valid operator; corrected here.*
