...
1. **Resident set present.** Confirm the project knowledge holds all of:
   `TSCG_ReferenceCorpus_Bootstrap.md`, `TSCG_ReferenceCorpus.md`,
   `_00_UserGuide/UserGuide.md`, `docs/reboot-kit/TSCG_FileTree.md`, and the
   `_00_UserGuide/exercises/**` files. Then, *reading from the Bootstrap* (not from
   memory), state: the REVOI expansion (what R stands for) and the three structural
   operators. → PASS if the files are present and the values match the Bootstrap.
...
4. **HEAD reachable (the lazy-load path).** Fetch
   ...
   and report its `version` value. → PASS if fetched; FAIL if you cannot reach it —
   that means **web fetch is OFF** and lazy-loading will require manual paste.
