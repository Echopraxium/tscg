...
def train_model(model, train_data: List[Tuple[torch.Tensor, torch.Tensor]],
    ...
    # Filtrer les entrées valides (forme correcte, tenseurs non nuls)
    valid_data = []
    for x, target in train_data:
        ...
        if x.numel() == 0 or target.numel() == 0:
            continue
        if x.shape[-1] != 20 or target.shape[-1] != 10:
            continue
    ...
    for epoch in range(epochs):
        ...
        for x, target in valid_data:
            ...
            if isinstance(model, StandardLLM):
                ...
                target_a = target[:, :5]
                target_r = target[:, 5:]
                loss = criterion(a_pred.squeeze(), target_a.squeeze()) + \
                       criterion(r_pred.squeeze(), target_r.squeeze())
            loss.backward()
            optimizer.step()
