...
def load_ontology(req: LoadRequest):
    ...
    # ── Extract subClassOf pairs from raw JSON-LD ──────────────────
    # Done here (not in /hierarchy) so the same expand_id() is used for
    # both object IDs and pair IRIs — guaranteeing byIri.has(child) matches
    # in ObjectExplorer regardless of rdflib IRI expansion differences.
    subclass_pairs    = []
    external_parents  = []   # parents not defined in this file (ghost nodes)
    known_ids         = {obj['id'] for obj in objects}
    if fmt == 'json-ld':
        try:
            for node in graph_nodes:
                child_iri = expand_id(node.get('@id', ''))
                if not child_iri:
                    continue
                sub = node.get('rdfs:subClassOf', None)
                if sub is None:
                    continue
                # subClassOf can be: string | {"@id": ...} | list of those
                parents_raw = sub if isinstance(sub, list) else [sub]
                for p in parents_raw:
                    if isinstance(p, str):
                        parent_iri = expand_id(p)
                    elif isinstance(p, dict):
                        parent_iri = expand_id(p.get('@id', ''))
                    else:
                        continue
                    if child_iri and parent_iri:
                        ...
                        if parent_iri not in known_ids and \
                            ...
                            # Derive local file path from parent IRI + @base
                            # e.g. https://.../ontology/M2_GenericConcepts.jsonld#Foo
                            #   →  <same dir as current file>/M2_GenericConcepts.jsonld
                            file_path_hint = None
                            try:
                                norm_base = base.rstrip('/')
                                if norm_base and parent_iri.startswith(norm_base):
                                    rel      = parent_iri[len(norm_base):].lstrip('/')
                                    file_rel = rel.split('#')[0]
                                    if file_rel:
                                        file_path_hint = str(
                                            Path(req.file_path).parent / file_rel
                                        ).replace('\\', '/')
                            except Exception:
                                pass
            ...
            print(f'[bridge] Warning extracting subclass pairs: {e}', flush=True)

    # ── Extract m3:instanceOf from root ontology node ─────────────
    # Used by Adjacent Layers tab (Option C) to show M2 metaclass links
    instanceof_targets = []
    if fmt == 'json-ld':
        try:
            M3_INSTANCE_OF_KEY = 'm3:instanceOf'
            for node in graph_nodes:
                node_type = node.get('@type', '')
                types = node_type if isinstance(node_type, list) else [node_type]
                if 'owl:Ontology' not in types:
                    continue
                raw = node.get(M3_INSTANCE_OF_KEY)
                if raw is None:
                    continue
                items = raw if isinstance(raw, list) else [raw]
                for item in items:
                    if isinstance(item, str):
                        target_iri = expand_id(item)
                    elif isinstance(item, dict):
                        target_iri = expand_id(item.get('@id', ''))
                    else:
                        continue
                    if target_iri:
                        lbl = target_iri.split('#')[-1].split('/')[-1]
                        # Derive file path hint
                        file_path_hint = None
                        try:
                            norm_base = base.rstrip('/')
                            if norm_base and target_iri.startswith(norm_base):
                                rel      = target_iri[len(norm_base):].lstrip('/')
                                file_rel = rel.split('#')[0]
                                if file_rel:
                                    file_path_hint = str(
                                        Path(req.file_path).parent / file_rel
                            ...
                            pass
                        instanceof_targets.append({
                            'id':             target_iri,
                            'label':          lbl,
                            'file_path_hint': file_path_hint,
                        })
                break  # only read root ontology node
        except Exception as e:
            print(f'[bridge] Warning extracting instanceOf: {e}', flush=True)
    ...
    return {
        ...
        'instanceof_targets': instanceof_targets,
    }
...
def _expand_value(val, ctx: dict, base: str):
    ...
    if isinstance(val, str):
        # Expand compact IRIs only — pattern: prefix:LocalName
        # where prefix is alphanumeric AND present in @context.
        # Plain text values (labels, comments, etc.) are returned as-is.
        if ':' in val and not val.startswith(('http://', 'https://')):
            colon_pos = val.index(':')
            prefix    = val[:colon_pos]
            if prefix and prefix.isalnum() and isinstance(ctx, dict) and prefix in ctx:
