                ...
                if let Some(op) = Opcode::from_byte(opcode_byte) {
                    ...
                    match op {
                        ...
                        Opcode::F_Call => {
                            ...
                            eprintln!("[tsk-link] FCALL reloc: obj[{}] sym=@{} sym_addr=0x{:X} obj_base=0x{:X} abs_code_off=0x{:X} instr_vm=0x{:X} delta={}",
...
mod tests {
    ...
    // ── NEW: test that trampoline reservation counts ALL libc relocs,
    //         not just those whose placeholder word happens to be 0x22.
    //         This catches the pre-scan bug where spill-store instructions
    //         before an F_CALL shifted local_off, causing the opcode check
    //         to see a D_MOV_I word instead of F_Call.
    #[test]
    fn test_trampoline_reservation_counts_all_libc_relocs() {
        use triskele_common::libc_symbols::LIBC_SYMBOLS;
        use triskele_common::isa::Opcode;
        use triskele_common::{encode_r, encode_i};
        ...
        // Build .tobj bytes with one reloc pointing to printf at fcall_local_off
        let mut tobj: Vec<u8> = Vec::new();
        tobj.extend_from_slice(b"TOBJ");
        let code_len = code.len() as u32;
        tobj.extend_from_slice(&code_len.to_le_bytes());  // code size
        tobj.extend_from_slice(&0u32.to_le_bytes());       // data size
        // symbols: "main" at offset 0 (code)
        tobj.extend_from_slice(&1u32.to_le_bytes());       // n_symbols
        tobj.extend_from_slice(b"main\0");
        tobj.extend_from_slice(&0u32.to_le_bytes());       // offset (code, bit31=0)
        // relocations: printf at fcall_local_off
        tobj.extend_from_slice(&1u32.to_le_bytes());       // n_relocs
        tobj.extend_from_slice(&fcall_local_off.to_le_bytes());
        tobj.extend_from_slice(b"printf\0");
        tobj.extend_from_slice(&code);

        let tmp = tempfile::NamedTempFile::new().unwrap();
        std::fs::write(tmp.path(), &tobj).unwrap();
        let obj = ObjectFile::load(&tmp.path().to_path_buf()).unwrap();
