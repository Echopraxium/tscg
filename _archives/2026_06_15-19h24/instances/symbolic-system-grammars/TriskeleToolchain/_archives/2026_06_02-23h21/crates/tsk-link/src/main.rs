        ...
        for (i, obj) in self.objects.iter().enumerate() {
            ...
            for (local_off, sym_name) in &obj.relocs {
                ...
                if let Some(op) = Opcode::from_byte(opcode_byte) {
                    // Look up symbol address (function or global data)
                    let sym_addr = self.global_syms.get(sym_name).copied()
                        .unwrap_or_else(|| {
                            log::warn!("reloc: symbol @{} not found", sym_name);
                            0
                        });
