# TriskeleToolchain — Handover v0.3.7
**Date**: 2026-06-09  
...
> ✅ Sextuple validation confirmée sur Windows (2026-06-09) avec `run_pipeline.py` v0.3.3 (`--release`).

---

## Changements en v0.3.7

### Fix 5 — `triskele-vm/src/memory/mod.rs` : sentinelle `0xDEADBEEF`

**Motivation** : La mémoire non-initialisée contenait des zéros, rendant les déréférencements de pointeurs invalides silencieux (comportement incorrect non diagnosticable, ex: binaire stale wolf3d lisait `tilemap` à zéro).

**Fix** : `Segment::new` remplace `vec![0u8; size]` par un remplissage cyclique du pattern little-endian `[0xEF, 0xBE, 0xAD, 0xDE]` (`0xDEADBEEF`). La mémoire chargée depuis `.tvmx` (code/data) écrase le poison normalement ; stack et heap restent poisonnés jusqu'à écriture explicite.

```rust
const DEADBEEF_PATTERN: [u8; 4] = [0xEF, 0xBE, 0xAD, 0xDE]; // little-endian

fn new(base: u64, size: usize) -> Self {
    let data: Vec<u8> = DEADBEEF_PATTERN
        .iter().cloned().cycle().take(size).collect();
    Self { base, data }
}
```

Nouveau test unitaire `test_deadbeef_poison` : vérifie que la mémoire non-écrite lit `0xDEAD_BEEF`, et qu'un `write` efface correctement le poison.

**Validation** : wolf3d avec binaire stale crashait immédiatement (détection immédiate) ; après `--clean` tous les 6 projets passent avec exits corrects.

---

### Fix 6 — `run_pipeline.py` v0.3.3 : `--release` + clean opt-in

**Problème** : `cargo clean` systématique (170MB supprimés, recompilation complète à chaque run) + `cargo run` sans `--release` (mode `dev` unoptimized).

**Fix** :
- Tous les `cargo run` passent en `--release`
- `--no-clean` supprimé → remplacé par `--clean` (opt-in)
- Par défaut : Cargo incrémental, build release

```
# Avant (v0.3.2) :  cargo clean systématique + cargo run dev
# Après (v0.3.3) :  cargo run --release, clean seulement si --clean
python run_pipeline.py test_main          # rapide (incrémental release)
python run_pipeline.py test_main --clean  # rebuild complet si nécessaire
```

---

### Fix 7 — `triskele-vm/src/main.rs` : alignement section Data

**Bug** : `main.rs` chargeait la section `.data` (globals comme `@tilemap`) à `load_ptr + ((code.len() + 7) & !7)` (aligné à 8), alors que `tsk-link` calcule `data_base = code_base + code_offset` avec `code_offset` aligné à 4. Si `code.len()` n'est pas multiple de 8 (ex: 1004 bytes → VM place data à +1008, tsk-link à +1004), décalage de 4 bytes → lecture dans la zone poison `0xDEADBEEF` au lieu des zéros initialisés.

**Détection** : rendu visible immédiatement grâce au poison `0xDEADBEEF` (Fix 5) — `IsSolid(0,0)` retournait 1 au lieu de 0, `SumTiles` donnait un résultat faux. Sans le poison, ce bug serait passé inaperçu (lecture de zéros par hasard).

**Fix** : `(code.data.len() as u64 + 7) & !7` → `(code.data.len() as u64 + 3) & !3`

```rust
// Avant (v0.3.6) — aligné à 8, diverge de tsk-link
let mut data_ptr = load_ptr + ((code.data.len() as u64 + 7) & !7);
// Après (v0.3.7) — aligné à 4, cohérent avec tsk-link
let mut data_ptr = load_ptr + ((code.data.len() as u64 + 3) & !3);
```

---

### Fix 8 — `triskele-vm/src/libc/mod.rs` : `printf` fonctionnel

**Avant** : stub debug qui affichait juste l'adresse du format string sur stderr et retournait 0.

**Après** : implémentation complète pour les patterns Wolf3D/Doom C89 :
- R0 = fmt_ptr, R1..R4 = args
- Spécificateurs : `%d` `%i` `%u` `%x` `%X` `%p` `%s` `%c` `%%`
- Retourne le nombre de caractères écrits dans R0
- 6 tests unitaires : `%d`, `%s`, `%x`, `%%`, no-args, entier négatif
...
## Fichiers modifiés en v0.3.7

```
crates/triskele-vm/src/memory/mod.rs  ← DEADBEEF poison pattern + test_deadbeef_poison
crates/triskele-vm/src/main.rs        ← alignement data_ptr: &!7 → &!3 (cohérence tsk-link)
crates/triskele-vm/src/libc/mod.rs   ← printf fonctionnel (%d %s %x %X %p %c %%)
run_pipeline.py                       ← v0.3.3 : --release partout, --clean opt-in
```
...
```
TriskeleToolchain/
  run_pipeline.py         ← v0.3.3 générique (lit pipeline.toml) — --release, clean opt-in
...
### Priorité 1 — ~~Sentinelle `0xDEADBEEF`~~ ✅ DONE (v0.3.7)

### Priorité 2 — `z_zone.c` complet (DoomGeneric)
Compiler le vrai `z_zone.c` avec stubs `I_Error`/`I_ZoneBase`. `printf` maintenant disponible (Fix 8). Nécessite des stubs pour `I_Error` (appelle `printf` + exit) et `I_ZoneBase` (retourne un buffer statique).

### Priorité 3 — tsk-libc
`malloc`, `free`, `printf`, `memset`, `memcpy` (stubs VM déjà présents pour memset/memcpy).

### Priorité 4 — ~~Fix `D_Shl` avec constante immédiate~~ ✅ ALREADY FIXED

Audit v0.3.7 : `codegen.rs` (ligne ~1101) et `tsk-link` émettent déjà `D_MovI R24, 16` + `D_Shl dst, dst, R24` avec `s2=R24` ≠ 0. La VM prend bien le chemin `regs.get(s2)`. Ce bug était présent dans une version antérieure, corrigé lors de l'implémentation des grandes adresses en v0.3.6. Aucune action requise.

### Priorité 5 — ~~Fix `tsk-asm` symtab format~~ ✅ ALREADY FIXED

Audit v0.3.7 : `tsk-asm/main.rs` (lignes 165-171) écrit `[name\0][u32 le]` et `tsk-link/main.rs` `parse_symtab()` lit exactement ce format. Aucune inversion présente. Aucune action requise.
...
### printf (v0.3.7)
- Max 4 args positionnels (R1..R4) — suffisant pour tout le code Doom/Wolf3D C89
- Output sur stderr (canal debug VM) — pas de stdout dans la VM
- `%p` formate comme `0x{:08x}` — cohérent avec les prints Doom (`I_Error`)
- Pas de support width/precision — non nécessaire pour les patterns C89 cibles

### Alignement data_ptr (v0.3.7)
- tsk-link aligne `code_offset` à 4 → `data_base` aligné à 4
- VM doit utiliser le même alignement pour que les adresses relocées correspondent
- Bug détecté grâce au poison 0xDEADBEEF : sans lui, lecture silencieuse de zéros

### DEADBEEF poison (v0.3.7)
- Pattern `[0xEF, 0xBE, 0xAD, 0xDE]` (little-endian) rempli à la création de chaque `Segment`
- La section `.tvmx` chargée écrase le poison → aucun impact sur les tests existants
- Stack (0x70000000) et heap (0x80000000) restent poisonnés jusqu'à écriture explicite
- Tout déréférencement de pointeur non-initialisé → valeur `0xDEADBEEFDEADBEEF` visible dans traces
...
- ~~Cas à corriger (Priorité 4)~~ : déjà corrigé — `codegen.rs` et `tsk-link` utilisent R24 comme s2
