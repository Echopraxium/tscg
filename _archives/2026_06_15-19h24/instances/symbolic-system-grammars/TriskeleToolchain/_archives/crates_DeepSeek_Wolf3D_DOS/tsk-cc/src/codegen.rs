    ...
    for block in &func.blocks {
        ...
        for instr in &block.instrs {
            ...
            match instr {
                ...
                Instr::Switch { .. } => {
                    // For liveness analysis, a switch behaves like multiple outgoing edges.
                    // We will ignore it here because the current implementation
                    // does not require precise liveness through switches.
                }
                _ => {}
            }
        }
    ...
    fn gen_instr(&mut self, instr: &Instr, cur_block: &str,
        ...
        match instr {
            ...
            Instr::Switch { cond, default_bb, cases } => {
                // Not yet implemented – log and emit a trap
                log::warn!("Switch instruction not implemented, emitting trap");
                self.em.emit(enc_r(Opcode::F_Trap, 0xCC, 0, 0, 0));
                // Fallback: jump to default block (unconditional)
