// uiControls.js - Contrôles UI et gestion des modes d'affichage
...
function createLayerGroups() {
  if (!layerCakeGroup) layerCakeGroup = new BABYLON.TransformNode("layerCakeGroup", scene);
  if (!planetGroup) planetGroup = new BABYLON.TransformNode("planetGroup", scene);
}

// Attache TOUS les éléments du Layer Cake au groupe racine
function attachAllToLayerCake() {
  if (!layerCakeGroup) layerCakeGroup = new BABYLON.TransformNode("layerCakeGroup", scene);
  const attach = (obj) => { if (obj && obj.parent !== layerCakeGroup) obj.parent = layerCakeGroup; };
...
function attachPlanetToGroup() {
  if (!planetGroup) planetGroup = new BABYLON.TransformNode("planetGroup", scene);
  if (planetEarth && planetEarth.parent !== planetGroup) planetEarth.parent = planetGroup;
  if (planetAtmosphere && planetAtmosphere.parent !== planetGroup) planetAtmosphere.parent = planetGroup;
}
...
function switchViewMode(mode) {
  SIM.viewMode = mode;
  if (mode === 'planet') {
    // Désactiver le Layer Cake
    if (layerCakeGroup) layerCakeGroup.setEnabled(false);
    if (planetGroup) planetGroup.setEnabled(true);
    ...
    // Créer la grille et le picker
    if (typeof createLatLongGrid !== 'undefined') {
      createLatLongGrid(6.05);
    }
    if (typeof createGridPicker !== 'undefined') {
      createGridPicker();
    }
    
    // RÉINITIALISER LE PANNEAU POUR LA GRILLE
    ...
    // Cacher le bouton sample par défaut (il sera affiché par selectGridCell)
    ...
    setTimeout(() => {
      addSampleMarker(0,-30); addSampleMarker(45,-40); addSampleMarker(-30,-110);
      addSampleMarker(20,70); addSampleMarker(65,-20);
    }, 1000);
  } else {
    // Nettoyer la sélection de grille
    if (typeof clearGridSelection !== 'undefined') clearGridSelection();
    if (gridPickerSphere) gridPickerSphere.dispose();
    
    // Désactiver la planète
    ...
    // RÉINITIALISER LE PANNEAU POUR LE LAYER CAKE
    if (SIM.selCell) {
...
function wireControls() {
  ...
  document.getElementById('btn-reset').addEventListener('click', () => {
    ...
    // Réinitialiser le seuil de séisme
    ...
    if (SIM.viewMode === 'layer') {
      camera.alpha = -Math.PI*.28; camera.beta = Math.PI*.30; camera.radius = 26; camera.target = new BABYLON.Vector3(0,-.5,0);
  ...
  // Wilson Cycle (seulement en mode Layer Cake)
  ...
  // Sliders (Convection, Slab density, Asymmetry, Propagation, Radius)
  ...
  // NOUVEAU : Curseur du seuil de séisme
  const thresholdSlider = document.getElementById('sl-threshold');
  ...
  // Bouton Toggle Grid (pour la grille méridiens/parallèles en mode Planète)
...
// Redimensionnement du splitter (barre verticale)
