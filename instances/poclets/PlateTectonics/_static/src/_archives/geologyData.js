// src/geologyData.js - Données géologiques et formes SVG par cellule
...
function generateCellSVG(lat, lon, isLand, shapeType, crustType) {
  ...
  // Couleur de fond selon le type de croûte
  let bgColor;
  switch(crustType) {
  ...
  if (isLand && shapeType) {
    // Dessiner la forme du continent
    ...
    // Ajouter des détails topographiques (montagnes)
  ...
  // Ajouter des marqueurs pour les zones actives
  if (crustType === 'ridge') {
...
// =============================================================
// INITIALISATION DES DONNÉES GÉOLOGIQUES
// =============================================================

function initGeologyData() {
  // Initialiser toutes les cellules avec des valeurs par défaut (océan)
...
// =============================================================
// DÉFINITION DES CONTINENTS PAR CELLULES
// =============================================================

const CONTINENTS = {
  'North America': {
    cells: [
      { lat: 45, lon: -105 }, { lat: 45, lon: -90 }, { lat: 45, lon: -75 },
      { lat: 30, lon: -105 }, { lat: 30, lon: -90 }, { lat: 30, lon: -75 },
      { lat: 60, lon: -120 }, { lat: 60, lon: -105 }, { lat: 60, lon: -90 },
      { lat: 15, lon: -90 }, { lat: 15, lon: -75 }
    ],
  ...
  'South America': {
    cells: [
      { lat: 0, lon: -60 }, { lat: 0, lon: -45 }, { lat: 0, lon: -30 },
      { lat: -15, lon: -60 }, { lat: -15, lon: -45 }, { lat: -15, lon: -30 },
      { lat: -30, lon: -60 }, { lat: -30, lon: -45 }
    ],
    type: CRUST_TYPES.CONTINENTAL,
    thickness: 36,
    shape: 'southAmerica'
  ...
  'Eurasia': {
    cells: [
      { lat: 45, lon: 0 }, { lat: 45, lon: 15 }, { lat: 45, lon: 30 }, { lat: 45, lon: 45 },
      { lat: 45, lon: 60 }, { lat: 45, lon: 75 }, { lat: 45, lon: 90 },
      { lat: 30, lon: 15 }, { lat: 30, lon: 30 }, { lat: 30, lon: 45 }, { lat: 30, lon: 60 },
      { lat: 30, lon: 75 }, { lat: 30, lon: 90 }, { lat: 60, lon: 0 }, { lat: 60, lon: 15 },
      { lat: 60, lon: 30 }, { lat: 60, lon: 45 }, { lat: 60, lon: 60 }, { lat: 60, lon: 75 }
    ],
    type: CRUST_TYPES.CONTINENTAL,
    thickness: 40,
    shape: 'eurasia'
  ...
  'Africa': {
    cells: [
      { lat: 15, lon: 0 }, { lat: 15, lon: 15 }, { lat: 15, lon: 30 },
      { lat: 0, lon: 0 }, { lat: 0, lon: 15 }, { lat: 0, lon: 30 },
      { lat: -15, lon: 0 }, { lat: -15, lon: 15 }, { lat: -15, lon: 30 }
    ],
    type: CRUST_TYPES.CONTINENTAL,
    thickness: 35,
  ...
  'Australia': {
    cells: [
      { lat: -15, lon: 120 }, { lat: -15, lon: 135 }, { lat: -15, lon: 150 },
      { lat: -30, lon: 120 }, { lat: -30, lon: 135 }, { lat: -30, lon: 150 }
    ],
    type: CRUST_TYPES.CONTINENTAL,
    thickness: 32,
    shape: 'australia'
  ...
  'Antarctica': {
    cells: [
      { lat: -75, lon: -180 }, { lat: -75, lon: -150 }, { lat: -75, lon: -120 },
      { lat: -75, lon: -90 }, { lat: -75, lon: -60 }, { lat: -75, lon: -30 },
      { lat: -75, lon: 0 }, { lat: -75, lon: 30 }, { lat: -75, lon: 60 },
      { lat: -75, lon: 90 }, { lat: -75, lon: 120 }, { lat: -75, lon: 150 }
    ],
  ...
  'Greenland': {
    cells: [
      { lat: 75, lon: -45 }, { lat: 75, lon: -30 }, { lat: 75, lon: -15 },
      { lat: 60, lon: -45 }, { lat: 60, lon: -30 }, { lat: 60, lon: -15 }
    ],
    type: CRUST_TYPES.CRATON,
    thickness: 32,
    shape: 'greenland'
...
// =============================================================
// ZONES ACTIVES (DORSALES, SUBDUCTIONS, HOTSPOTS, RIFTS)
// =============================================================

const RIDGES = [
  { lat: 0, lon: -30, range: 10, name: 'Mid-Atlantic Ridge', type: CRUST_TYPES.RIDGE },
  { lat: 45, lon: -40, range: 8, name: 'North Atlantic Ridge', type: CRUST_TYPES.RIDGE },
  { lat: -30, lon: -110, range: 10, name: 'East Pacific Rise', type: CRUST_TYPES.RIDGE },
  { lat: 20, lon: 70, range: 8, name: 'Indian Ocean Ridge', type: CRUST_TYPES.RIDGE },
  { lat: 65, lon: -20, range: 6, name: 'Iceland Ridge', type: CRUST_TYPES.RIDGE }
];

const SUBDUCTIONS = [
  { lat: 35, lon: 140, range: 12, name: 'Japan Trench', type: CRUST_TYPES.SUBDUCTION },
  { lat: -20, lon: -70, range: 10, name: 'Peru-Chile Trench', type: CRUST_TYPES.SUBDUCTION },
  { lat: -30, lon: 180, range: 10, name: 'Tonga Trench', type: CRUST_TYPES.SUBDUCTION },
  { lat: 40, lon: 25, range: 8, name: 'Hellenic Arc', type: CRUST_TYPES.SUBDUCTION },
  { lat: 10, lon: 125, range: 8, name: 'Philippine Trench', type: CRUST_TYPES.SUBDUCTION }
];

const HOTSPOTS = [
  { lat: 19, lon: -155, range: 5, name: 'Hawaii Hotspot', type: CRUST_TYPES.HOTSPOT },
  { lat: 44, lon: -110, range: 5, name: 'Yellowstone Hotspot', type: CRUST_TYPES.HOTSPOT },
  { lat: -20, lon: -175, range: 5, name: 'Tonga Hotspot', type: CRUST_TYPES.HOTSPOT },
  { lat: 37, lon: 14, range: 4, name: 'Etna Hotspot', type: CRUST_TYPES.HOTSPOT }
];

const RIFTS = [
  { lat: 10, lon: 40, range: 6, name: 'East African Rift', type: CRUST_TYPES.RIFT },
  { lat: 50, lon: 100, range: 6, name: 'Baikal Rift', type: CRUST_TYPES.RIFT },
  { lat: 35, lon: -110, range: 5, name: 'Rio Grande Rift', type: CRUST_TYPES.RIFT }
];
...
function loadGeologyData() {
  ...
  // Charger les dorsales
  for (const ridge of RIDGES) {
    applyToCell(ridge.lat, ridge.lon, ridge);
  }
  
  // Charger les zones de subduction
  for (const sub of SUBDUCTIONS) {
    applyToCell(sub.lat, sub.lon, sub);
  }
  
  // Charger les points chauds
  for (const hot of HOTSPOTS) {
    applyToCell(hot.lat, hot.lon, hot);
  }
  
  // Charger les rifts
  for (const rift of RIFTS) {
    applyToCell(rift.lat, rift.lon, rift);
  }
  
  console.log("Geology data loaded for", Object.keys(GEOLOGY_DATA).length, "cells");
}

// =============================================================
// GÉNÉRATION DES FORMES SVG PAR CELLULE
// =============================================================

function generateAllCellShapes() {
  for (let lat = -90; lat <= 90; lat += 15) {
    for (let lon = -180; lon <= 180; lon += 15) {
      const key = `${lat},${lon}`;
      const data = GEOLOGY_DATA[key];
      if (data) {
        const svg = generateCellSVG(lat, lon, data.isLand, data.shapeType, data.crustType);
        CELL_SHAPES[key] = {
          svg: svg,
          isLand: data.isLand,
...
function getCellShape(lat, lon) {
  ...
  const key = `${latCell},${lonCell}`;
  return CELL_SHAPES[key] || { svg: null, isLand: false, shapeType: null, crustType: CRUST_TYPES.OCEANIC };
}
