...
// Waveform selector (4 clickable decals)
let currentWaveform = 0; // 0=sine, 1=triangle, 2=square, 3=sawtooth
const waveformNames = ['sine', 'triangle', 'square', 'sawtooth'];
let waveformFrames = [];  // White frames to highlight active waveform
let updateLcdWaveform = null;  // Function to update LCD waveform display
...
function createThereminModel() {
  ...
  // LOCAL AXES for panel (X=red, Y=green, Z=blue) - HIDDEN
  ...
  // Waveform indicator on LCD (top-left corner)
  const lcdWaveformPlane = BABYLON.MeshBuilder.CreatePlane(
    'lcdWaveformDisplay',
    { width: 0.12, height: 0.12 },
    scene
  );
  lcdWaveformPlane.position = new BABYLON.Vector3(-0.18, 0.18, controlDepth + 0.02);
  
  const lcdWaveformTexture = new BABYLON.DynamicTexture('lcdWaveformTexture', 256, scene);
  const lcdWaveformMat = new BABYLON.StandardMaterial('lcdWaveformMat', scene);
  lcdWaveformMat.diffuseTexture = lcdWaveformTexture;
  lcdWaveformMat.emissiveColor = new BABYLON.Color3(1, 1, 1);  // White to show texture colors
  lcdWaveformMat.backFaceCulling = false;
  lcdWaveformMat.alpha = 1.0;
  lcdWaveformPlane.material = lcdWaveformMat;
  lcdWaveformPlane.parent = panel;
  
  // Function to update LCD waveform display
  updateLcdWaveform = () => {
    const waveformSymbols = ['∿', '△', '⊓', '⊿'];  // ∿ = proper sine wave symbol
    const ctx = lcdWaveformTexture.getContext();
    
    // Draw LCD-matching background (using emissive color: 0.1, 0.3, 0.2)
    ctx.fillStyle = 'rgb(' + 
      Math.round(0.1 * 255) + ',' + 
      Math.round(0.3 * 255) + ',' + 
      Math.round(0.2 * 255) + ')';
    ...
    ctx.fillText(waveformSymbols[currentWaveform], 128, 128);
    lcdWaveformTexture.update();
  };
  updateLcdWaveform();  // Initial display
  ...
  // === WAVEFORM SELECTOR (4 clickable decals only) ===
  const waveformKnobX = -0.7;  // Position on LEFT side of panel
  
  // NO KNOB - just the 4 decals below
  
  // 4 waveform decals around the knob
  const waveformDecals = [
    { angle: -135, text: '∿', color: new BABYLON.Color3(0.3, 0.7, 1.0) },  // Sine wave symbol
  ...
  const decalRadius = 0.12;  // Closer to center
  const decalSize = 0.20;    // Larger decals
  const decalCenterY = 0.1;  // Higher on panel to avoid overlap with wood face
  ...
  waveformDecals.forEach((decal, i) => {
    ...
    // White frame for active selection (behind the decal)
    const frame = BABYLON.MeshBuilder.CreatePlane(
      `waveformFrame${i}`,
      { width: decalSize + 0.02, height: decalSize + 0.02 },
      scene
    );
    frame.position = new BABYLON.Vector3(waveformKnobX + x, decalCenterY + y, controlDepth + 0.12);  // Further forward
    const frameMat = new BABYLON.StandardMaterial(`frameMat${i}`, scene);
    frameMat.emissiveColor = new BABYLON.Color3(1, 1, 1);  // White
    frameMat.backFaceCulling = false;
    frame.material = frameMat;
    frame.parent = panel;
    frame.isVisible = (i === currentWaveform);  // Only show for active waveform
    waveformFrames.push(frame);
    
    // Create text plane for waveform symbol
    const textPlane = BABYLON.MeshBuilder.CreatePlane(
      `waveformDecal${i}`,
      { width: decalSize, height: decalSize },
      scene
    );
    textPlane.position = new BABYLON.Vector3(waveformKnobX + x, decalCenterY + y, controlDepth + 0.13);  // In front of frame
    
    // Make decal clickable
    textPlane.isPickable = true;
    textPlane.waveformIndex = i;  // Store which waveform this represents (0=sine, 1=triangle, 2=square, 3=sawtooth)
    
    // Create dynamic texture for text
    const textTexture = new BABYLON.DynamicTexture(`waveformText${i}`, 512, scene);  // Higher resolution for larger symbols
    ...
    textMat.alpha = 1.0;  // Fully opaque
    textPlane.material = textMat;
    textPlane.parent = panel;
    
    // Draw text on texture - LARGER and CENTERED
    const ctx = textTexture.getContext();
    ctx.fillStyle = 'white';
    ctx.font = 'bold 300px Arial';  // Much larger font
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(decal.text, 256, 256);  // Centered in 512x512 texture
    textTexture.update();
  });
  
  // NO POSITION MARKERS - decals are self-explanatory
...
function updateWaveform() {
  ...
  // Update white frame visibility (show only for active waveform)
  if (waveformFrames && waveformFrames.length === 4) {
    waveformFrames.forEach((frame, i) => {
      frame.isVisible = (i === currentWaveform);
    });
  }
  
  // Update LCD waveform display
  if (updateLcdWaveform) {
    updateLcdWaveform();
  }
...
function setupPicking() {
  ...
  canvas.addEventListener('pointerdown', (evt) => {
    ...
    console.log('🖱️ CLICK at', evt.clientX, evt.clientY);
    if (pickResult.hit) {
      console.log('  ✓ Hit:', pickResult.pickedMesh.name, '| isPickable:', pickResult.pickedMesh.isPickable);
    } else {
      console.log('  ✗ No hit');
    }
    ...
    if (pickResult.hit) {
      ...
      // Click on WAVEFORM DECAL - select waveform directly
      else if (pickResult.pickedMesh && pickResult.pickedMesh.name.startsWith('waveformDecal')) {
        const waveformIndex = pickResult.pickedMesh.waveformIndex;
        console.log('🎛️ WAVEFORM DECAL CLICKED!');
        console.log('  - Picked mesh:', pickResult.pickedMesh.name);
        console.log('  - Current waveform:', currentWaveform, '(' + waveformNames[currentWaveform] + ')');
        console.log('  - Selected waveform:', waveformIndex, '(' + waveformNames[waveformIndex] + ')');
        
        currentWaveform = waveformIndex;  // Set waveform directly
        
        updateWaveform();
      }
      // Click on WAVEFORM KNOB - cycle through 4 positions (fallback)
      else if (pickResult.pickedMesh && 
               (pickResult.pickedMesh.name === 'waveformKnob' || 
                pickResult.pickedMesh.name === 'waveformKnobVisible')) {
        console.log('🎛️ WAVEFORM KNOB CLICKED!');
        console.log('  - Picked mesh:', pickResult.pickedMesh.name);
        console.log('  - Current waveform:', currentWaveform, '(' + waveformNames[currentWaveform] + ')');
        
        currentWaveform = (currentWaveform + 1) % 4; // Cycle: 0→1→2→3→0
        
        console.log('  - New waveform:', currentWaveform, '(' + waveformNames[currentWaveform] + ')');
        updateWaveform();
      }
    }
  });
...
function setupAudioSystem() {
  ...
  document.getElementById('waveform-select').addEventListener('change', (e) => {
    // Update currentWaveform based on HTML select value
    const waveformType = e.target.value;
    currentWaveform = waveformNames.indexOf(waveformType);
    
    if (oscillator && isAudioInitialized) {
      oscillator.type = waveformType;
    }
    
    // Update visual indicators (knob + LCD)
    updateWaveform();
  });
