...
function updateChips() {
  ...
  const labels = { ollama: '🏠 Ollama', deepseek: '☁️ DeepSeek', anthropic: '⭐ Claude API' };
...
async function renderProviderList() {
  const providers = await window.tscgMiner.config.listProviders();
  ui.providerList.innerHTML = '';

  for (const p of providers) {
    const row = document.createElement('div');
    row.className = 'provider-row' + (p.id === state.config?.active ? ' selected' : '');
    ...
    row.innerHTML = `
      ...
      <div class="provider-info">
        ...
        <div class="provider-meta">Quality: ${p.quality} · ${p.speed}</div>
      </div>
  ...
  showKeySection(state.config?.active ?? 'ollama');
}
...
function showKeySection(pid) {
  ['ollama', 'deepseek', 'anthropic'].forEach(id => {
...
async function loadSettings() {
  ...
  ui.inputOllamaModel.value    = p.ollama?.model ?? '';
...
ui.btnSaveSettings.addEventListener('click', async () => {
  ...
  state.config.providers.ollama.model         = ui.inputOllamaModel.value.trim() || 'phi3.5:3.8b-mini-instruct-q4_K_M';
...
async function testConnection(pid) {
  ...
  state.config.providers.ollama.model       = ui.inputOllamaModel.value.trim() || state.config.providers.ollama.model;
  state.config.providers.deepseek.apiKey    = ui.inputDeepseekKey.value.trim();
  state.config.providers.anthropic.apiKey   = ui.inputAnthropicKey.value.trim();
  state.config.providers.anthropic.model    = ui.inputAnthropicModel.value;
...
window.tscgMiner.rag.onProgress(msg => {
  log(msg, 'log-info');
  ui.ragStatusLine.textContent = msg;
  ui.ragStatusLine.style.color = 'var(--muted)';
});
...
ui.btnBuildRag.addEventListener('click', async () => {
  ...
  if (result.ok) {
    state.ragBuilt = true;
    ui.ragStatusLine.textContent = `✓ ${result.files} files · ${result.chunks} chunks · ${result.errors} errors`;
    ui.ragStatusLine.style.color = 'var(--accent)';
    log(`RAG ready — ${result.chunks} chunks`, 'log-ok');
    setStatus(`RAG ready — ${result.chunks} chunks`);
  } else {
