// =============================================================
// TscgOntologyEditor — src/main.js   (CJS)
...
const PLUGINS_PATH   = path.join(LOCAL_APP_DATA, 'TscgOntologyEditor', 'plugins')
...
function createWindow () {
  ...
  mainWindow.webContents.on('did-finish-load', () => {
    ...
    // Read preconfigured layer paths from M0_TscgOntologyEditor.jsonld.
    // Cached in config.json — M0 re-read only if its mtime has changed.
    const m0Path     = path.join(__dirname, '..', 'M0_TscgOntologyEditor.jsonld')
  ...
  const template = [
    ...
    // ── Help ──────────────────────────────────────────────────
    {
      ...
      submenu: [
        ...
        {
          label: 'TSCG Documentation…',
          click: () => require('electron').shell.openExternal('https://echopraxium.github.io/tscg/')
        },
        { type: 'separator' },
        {
          ...
          click: () => {
            dialog.showMessageBox(mainWindow, {
              type:    'info',
              title:   'About TscgOntologyEditor',
              message: 'TscgOntologyEditor',
