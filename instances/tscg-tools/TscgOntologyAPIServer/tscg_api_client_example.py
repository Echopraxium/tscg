...
class TscgAPIClient:
    ...
    def get_tscg_tools(self) -> list[dict]:
        ...
        WHERE {
            ?tool m3:ontologyType m3:TscgTool .
...
def demo(client: TscgAPIClient, ontology_dir: Optional[str] = None):
    ...
    for g in graphs[:10]:  # Show first 10
        name = g.get("name", "?").split("/")[-1]
        triples = g.get("triple_count", "?")
        print(f"  {name:45s} {triples:>6} triples")
    if len(graphs) > 10:
    ...
    for c in concepts[:5]:
        ...
        formula = c.get("formula", {}).get("value", "—")
    ...
    scores_sorted = sorted(
        scores,
        key=lambda x: float(x.get("A", {}).get("value", 0)),
        reverse=True
    )
    print(f"  {'Instance':30s} {'A':5s} {'S':5s} {'F':5s} {'It':5s} {'D':5s}")
    for s in scores_sorted[:5]:
        label = s.get("label", {}).get("value", s.get("instance", {}).get("value", "?").split("/")[-1])
        vals = [round(float(s.get(k, {}).get("value", 0)), 2) for k in ["A","S","F","It","D"]]
    ...
    for b in bio:
        ...
        itype = b.get("type", {}).get("value", "?").split("#")[-1]
    ...
    SELECT ?label ?gap ?spectral
    WHERE {
        ?inst m0:epistemicGap ?gap ;
              rdfs:label ?label .
    ...
    for row in rows:
        ...
        gap     = float(row.get("gap", {}).get("value", 0))
        spectral = row.get("spectral", {}).get("value", "?")
