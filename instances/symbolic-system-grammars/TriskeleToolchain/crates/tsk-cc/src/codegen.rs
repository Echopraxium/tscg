...
impl<'m> FuncGen<'m> {
    fn new(func: &'m Function, globals: &'m HashMap<String, usize>) -> Self {
        ...
        // Stack-based: alloca addresses are always SP+slot — no register pinning needed.
        // Use liveness-based allocation for SSA temporaries (max register reuse).
        ra.has_alloca = false;
        let _ = has_alloca;
        Self {
            func, ra,
    ...
    fn load_reg_or_imm(&mut self, val: &Value, scratch: usize) -> Result<usize> {
        match val {
            Value::Reg(name) => {
                // Stack-based: if name is an alloca slot, recompute SP+slot each time
                if let Some(&slot) = self.alloca_slots.get(name) {
                    self.em.emit(enc_i(Opcode::D_MovI, scratch as u8, slot));
                    self.em.emit(enc_r(Opcode::D_Add,
                        scratch as u8, REG_SP as u8, scratch as u8, 0));
                    return Ok(scratch);
                }
                self.ra.get(name)
            }
